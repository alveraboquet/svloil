...
$possible_plugin_locations = array(
	...
	'/var/www/vhosts/svloil.nl/httpdocs/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://svloil.nl/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/var/www/vhosts/svloil.nl/httpdocs/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-svloil.nl.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-svloil.nl.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-svloil.nl.php');
}
